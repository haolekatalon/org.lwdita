...
<xsl:param name="OUTPUTDIR"/>
 ...
 <xsl:template match="*" mode="process.note.danger">
  ...
  <!-- For Steps ol -->
  <xsl:template match="*[contains(@class, ' task/steps ') and
                         contains(@class, ' topic/ol ')]" name="topic.ol.steps">
    <xsl:variable name="olcount" select="count(ancestor-or-self::*[contains(@class, ' topic/ol ')])"/>
    <xsl:apply-templates select="*[contains(@class, ' ditaot-d/ditaval-startprop ')]" mode="out-of-line"/>
    <orderedlist issteps="">
      <xsl:call-template name="commonattributes"/>
 ...
 <!-- Custom: Topic/step -->
  ...
  <xsl:template match="*[contains(@class, ' topic/li ') and contains(@class, ' task/step ')]" name="topic.li.step">
  <li isstep="">
    <xsl:choose>
      <xsl:when test="parent::*/@compact = 'no'">
        <xsl:attribute name="class">liexpand</xsl:attribute>
