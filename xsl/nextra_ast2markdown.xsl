...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="header" mode="ast">
    <!-- Custom: Add imports -->
    <!-- <xsl:if test="@level='1'">
      <xsl:text>import &#123; Steps, Callout &#125; from 'nextra-theme-docs'</xsl:text>
      <xsl:value-of select="$linefeed"/>
      <xsl:text>import &#123; Screenshot &#125; from 'components/screenshot'</xsl:text>
      <xsl:value-of select="$linefeed"/>
      <xsl:value-of select="$linefeed"/>
    </xsl:if> -->
